...
try:
    from core.agents.crew_orchestrator import RecruitAICrew, run_recruitment_crew
    CREWAI_AVAILABLE = True
except ImportError:
    CREWAI_AVAILABLE = False
    print("Warning: CrewAI not available, multi-agent features disabled")

load_dotenv()

BASE_DIR = Path(__file__).resolve().parent
_cv_cache: dict = {}  # key: (filename, mtime) → candidate dict
_pending_analyses: dict = {}  # key: analysis_id → analysis data
...
def analyze_crew(request: AnalyzeRequest) -> dict:
    """
    Analyze candidates using the multi-agent CrewAI system.
    Returns pending groups awaiting confirmation before scheduling.
    """
    if not CREWAI_AVAILABLE:
        raise HTTPException(
            status_code=503,
            detail="CrewAI not available. Please install crewai and dependencies."
        )
    ...
    try:
        ...
        return {
            ...
            "status": "pending_confirmation",
            "message": "Analysis complete. Awaiting confirmation to schedule meetings.",
            "result": result,
            "stats": {
...
def confirm_and_schedule(analysis_id: str) -> dict:
    """
    Confirm the grouped teams and schedule meetings on Google Calendar.
    """
    if not CREWAI_AVAILABLE:
        raise HTTPException(
            status_code=503,
            detail="CrewAI not available."
        )
